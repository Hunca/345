...
void playerTurn(Ball ball, sf::CircleShape test)
{
...
int main() {
    ...
    for(int i = 0; i < 2; i++) {
        balls[i] = new Ball(1, 20.f, 425.f, 425.f);
    }
    
    sf::CircleShape test(balls[0]->getRadius());
    test.setPosition(balls[0]->x, balls[0]->y);
    while (window.isOpen())
	{
		sf::Event event;
		while (window.pollEvent(event))
